...
import pandas as pd
array_optimization_name = 'CMIM'
subspace_transfer_learning_algorithm = 'mRMR'
import numpy as np
...
from transfer_learning.transfer_algorithm import CSDL
from sklearn.svm import SVC
...
# lambdas_grid = range(1,1000)
n_neighbors_grid= [1,50] #range(1,50)
...
for feature_num in range(num_fea,num_fea+1):
    ...
    csdl = CSDL(X_S_selected,Y_S,X_T_selected,Y_T)
    max_acc_after_projection = -1
    for single_parameter in generated_grid:#对超参数lamba的调试
        alpha,n_neighbors = single_parameter[0],single_parameter[1]
        eig_vec = csdl.fit_transform(alpha,n_neighbors)
...
plt.title('CSDL+mRMR(1NN分类器)XS_中值')
