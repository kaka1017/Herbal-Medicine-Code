...
import pandas as pd
array_optimization_name = 'JMI'
subspace_transfer_learning_algorithm = 'LDSP'
import numpy as np 
...
from transfer_learning.transfer_algorithm import LDSP_constrained
from sklearn.svm import SVC
...
for feature_num in range(num_fea,num_fea+1):
    ...
    ldsp = LDSP_constrained(X_S_selected,Y_S,X_T_selected,Y_T)
    ...
    for single_parameter in generated_grid:#对超参数lamba的调试
        ...
        eig_vec = ldsp.fit_transform(alpha,lambdas,detla,beta)
...
print("第一篇对比LDSP算法迁移精度",max_acc_after_projection)
...
plt.title('LDSP+JMI(1NN分类器)XS_中值')
