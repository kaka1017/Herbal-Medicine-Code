...
def conditioned_hisc(data_Y,data_X,data_Z,kernel_type = 'linear',gamma = 1):
    ...
    Kx = kernel(kernel_type, XZ, None,gamma) 
    Ky = kernel(kernel_type, YZ, None, gamma) 
    Kz = kernel(kernel_type, data_Z, None, gamma) 
    n_sample = data_X.shape[1]
    H = np.identity(n_sample) - 1/n_sample*np.ones([n_sample,1])@np.ones([1,n_sample])
    KX = Kx@H
    KY = Ky@H
    ...
    beta_z = n_sample**2 / (matrix_norm**2) 
...
for gamma in [8]:  
    ...
    for i in range(num_fea):
        hisc_value[i] = my_hsic(X_S_new[:,i].reshape(1,-1), Y_S_unified.reshape(1,-1)/6,'rbf',gamma=gamma) 
    ...
    F = [i for i in range(num_fea)] 
    S = list()
    f0 = np.argmax(hisc_value)
    F.remove(f0)
    S.append(f0)
    for k in range(1,num_fea): 
        List = -5000*np.ones(num_fea)
        l1 = len(S) 
        l2 = len(F) 
        min_value = 5000
        for i in range(l2): 
            mi_sy = hisc_value[F[i]] 
            redun = 0
            min_value = 5000
            for j in range(l1):
                cmi_sfy = condition_hisc[F[i]][S[j]]
    ...
    plt.title('my_alg+HSIC(CMIM)_rbf(1NN分类器)XS_中值(WSP2110置零)'+str(gamma))
